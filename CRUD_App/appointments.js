...
router.post('/appointments', (req, res) => {
  const { user_id, center_id, timeslot_id, date } = req.body;
  ...
  connection.query('SELECT COUNT(*) AS existingAppointments FROM rendez_vous WHERE user_id = ? AND date = ?', [user_id, date], (err, existingAppointmentResults) => {
    ...
    connection.query('SELECT COUNT(*) AS total FROM rendez_vous WHERE center_id = ? AND timeslot_id = ?', [center_id, timeslot_id], (err, countResults) => {
      ...
      connection.query('SELECT capacite FROM centres_sante WHERE id = ?', [center_id], (err, capResults) => {
        ...
        connection.query('INSERT INTO rendez_vous (user_id, center_id, timeslot_id, date, status) VALUES (?, ?, ?, ?, ?)', [user_id, center_id, timeslot_id, date, status], (err, insertResult) => {
          if (err) {
...
router.delete('/appointments/:id', (req, res) => {
  ...
  console.log('ID du rendez_vous à annuler :', appointmentId); // Ajout d'un message de console pour vérification
  connection.query('DELETE FROM rendez_vous WHERE id = ?', [appointmentId], (err, result) => {
    ...
    console.log('ID du rendez-vous à annuler :', appointmentId); // Ajout d'un message de console pour vérification

    console.log('Rendez-vous annulé avec succès.'); // Ajout d'un message de console pour vérification
    console.log('ID du rendez-vous à annuler :', appointmentId); // Ajout d'un message de console pour vérification
